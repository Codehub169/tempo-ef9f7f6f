...
class GitPullFailedError(Exception):
    ...
    def __str__(self):
        ...
        if self.returncode is not None:
            ...
            # Explicitly check length after stripping for boolean condition
            has_stderr_content = isinstance(self.stderr, str) and len(self.stderr.strip()) > 0
            has_stdout_content = isinstance(self.stdout, str) and len(self.stdout.strip()) > 0
            ...
            if not has_stderr_content and not has_stdout_content:
                raw_stderr_repr = repr(self.stderr if self.stderr is not None else None)
            ...
            elif stdout_content_on_other_error:
